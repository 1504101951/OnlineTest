<template>
  <h1 style="margin: 0 auto 10px auto; width: 50px; padding: 0">
    {{ class_name }}
  </h1>
...
export default {
  ...
  methods: {
    ...
    findUser(newPage) {
      this.$axios.get('admin/classDetails', {
        params: {
          account: localStorage.getItem('account'),
          class_id: this.rowData.id,
          page: newPage,
          limit: this.limit
        }
      })
  ...
  created() {
    this.$axios.get('/admin/classDetails', {
      params: {
        ...
        .then(res => {
          this.class_name = res.data.class_info.class_name
          this.currentPage = res.data.page
          this.total = res.data.total
          this.items = res.data.result
        })
